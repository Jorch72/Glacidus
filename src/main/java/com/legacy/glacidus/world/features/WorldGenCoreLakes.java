...
public class WorldGenCoreLakes extends WorldGenerator
{

    private final Block block;

    public WorldGenCoreLakes(Block blockIn)
    {
        this.block = blockIn;
    }
                ...
                for (int j3 = 0; j3 < 16; ++j3)
                {
                    for (int j4 = 4; j4 < 8; ++j4)
                    {
                        if (aboolean[(i2 * 16 + j3) * 8 + j4])
                        {
                            ...
                            {
                                worldIn.setBlockState(blockpos, BlocksGlacidus.lumicia_grass.getDefaultState(), 2);
                            }
                        }
                    }
                }
